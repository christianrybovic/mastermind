...
class ENDPOINT():
    ...
    ROOM_CREATE = URL + "room/create"
    ROOM_CONFIG = URL + "room/config"
    ROOM_JOIN = URL + "room/join"
    ROOM_LEAVE = URL + "room/leave"
    ROOM_STATE = URL + "room/state"
...
class Restapi:
    ...
    def game_guess(self, combination: str, is_multiplayer=False) -> Turn:
        data = {
            "combination": combination
        }
        if is_multiplayer:
            text = ["Waiting for the last brain cell to clock in...",
                    "Waiting for the rest of the geniuses to submit their guesses...",
                    "Please wait while the other players pretend they know the answer...",
                    "The guessing goblins are hard at work. Please wait."]
    ...
    def room_create(self) -> int:
        print(LOADING_TEXT)
        response = self.session.post(ENDPOINT.ROOM_CREATE)
        response.raise_for_status()

    def room_join(self, room_id: int) -> bool:
        data = {
            "roomId": room_id
        }
        print(LOADING_TEXT)
        response = self.session.post(ENDPOINT.ROOM_JOIN, json=data)
    ...
    def room_state(self) -> Room:
        response = self.session.get(ENDPOINT.ROOM_STATE)
        response.raise_for_status()
        data = response.json()
        players: dict[str, Player] = {}
        for player in data["players"]:
            players[player["userId"]] = Player(player["userName"], player["correctPosition"], player["correctColor"])
        return Room(data["roomId"], data["action"], data["allowDuplicates"], players)

    def room_config(self, allow_duplicates: bool, start=False) -> bool:
        data = {
            "allowDuplicates": allow_duplicates,
            "start": start
        }
        print(LOADING_TEXT)
        response = self.session.post(ENDPOINT.ROOM_CONFIG, json=data)
        response.raise_for_status()
